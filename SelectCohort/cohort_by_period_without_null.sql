-- FUNCTION: public.internacao_com_diagnostico(character varying, character varying)

-- DROP FUNCTION public.internacao_com_diagnostico(character varying, character varying);

CREATE OR REPLACE FUNCTION public.internacao_com_diagnostico(
  		...
  		CURSOR(c_dt_ini date, c_dt_fim date) FOR
			select  inter.person_id, inter.visit_occurrence_id, row_to_json(inter) as inter_json
					from (select vo.person_id,
							  ...
							  join cdm5.condition_occurrence co1
							    on co1.visit_occurrence_id = vo.visit_occurrence_id
...
ALTER FUNCTION public.internacao_com_diagnostico(character varying, character varying)
    OWNER TO omop;
